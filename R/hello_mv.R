...
x <- matrix(sort(runif(n = 100,min = -pi,max = pi))) %>% as.data.frame()
x_test <- matrix(sort(runif(n = 150,min = -pi,max = pi))) %>% as.data.frame()
...
comparison <- microbenchmark::microbenchmark(bart2(x_train = x,y = unlist(c(y)),x_test = x_test,
                                                                n_tree = 200,n_mcmc = 2500,alpha = 0.95,
                                                                beta = 2,tau = diff(range(y)),node_min_size = 1,
                                                                df = 3,sigquant = 0.9,stump = FALSE,
                                                                n_burn = 500,scale_bool = TRUE),
                                             dbarts::bart(x.train = x,y.train = unlist(c(y)),ntree = 200,x.test = x_test,keeptrees = TRUE),
                                             times = 1)
